...
pub struct GroundSheet(pub Handle<TextureAtlas>);
...
pub fn load_spritesheets(
    ...
    let texture_ground_handle = asset_server.load("textures/ground.png");
    ...
    let texture_atlas_ground =
        TextureAtlas::from_grid(texture_ground_handle, Vec2::new(16.0, 16.0), 10, 10);
    ...
    let texture_atlas_ground_handle = texture_atlases.add(texture_atlas_ground);
    commands.insert_resource(GabeSheet(texture_atlas_gabe_handle));
    commands.insert_resource(ManiSheet(texture_atlas_mani_handle));
    commands.insert_resource(GroundSheet(texture_atlas_ground_handle));
}
